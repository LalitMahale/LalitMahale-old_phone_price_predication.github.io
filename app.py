from flask import Flask, render_template, request
import pickle 
import numpy as np
import requests


app = Flask(__name__,static_folder = "statics")
model = pickle.load(open("Price_Predictor.pkl","rb"))
scaler = pickle.load(open("Scaler.pkl","rb"))

@app.route("/",method = ["GET"])
def home():
    return render_template("index.html")

@app.route("/predict/",method = ["GET","POST"])
def predict():
    if requests.method = "POST":
        screen_size = float(requests.form["screen_size"])
        screen_size = np.log(screen_size)
        four_G = requests.form["4g"]
        four_G = [1 if four_G == "Yes" else 0]
        five_G = requests.form["5g"]
        five_G = [1 if five_G == "Yes" else 0]
        rear_camera = float(requests.form["rear_camera_mp"])
        rear_camera = rear_camera**(0.5)
        front_camera = float(requests.form["front"])
        front_camera = np.log(front_camera)
        internal_memory = int(requests.form["internal_memory"])
        ram = float(requests.form["ram"])
        battery = int(requests.form["battery"])
        battery = np.log(battery)
        weight = float(requests.form["weight"])
        weight = 1/weight
        released_year = requests.form["release_year"]
        year_2014,year_2015,year_2016,year_2017,year_2018,year_2019,year_2020 = 0,0,0,0,0,0,0
        
        if released_year == "2014":
            year_2014 = 1
        elif released_year == "2015":
            year_2015 = 1
        elif released_year == "2016":
            year_2016 = 1
        elif released_year == "2017":
            year_2017 = 1
        elif released_year == "2018":
            year_2018 = 1
        elif released_year == "2019":
            year_2019 = 1
        elif released_year == "2020":
            year_2020 = 1

        days_used = int(request.form['days_used'])
        new_price = float(request.form['normalized_new_price'])
        normalized_new_price = np.log(new_price)
        brand_dict = {'Acer': 4.294424174,'Alcatel': 4.026422546,'Apple': 5.011901159,'Asus': 4.4673986215,'BlackBerry': 4.2931666795000005,
                      'Celkon': 3.116621591,'Coolpad': 4.243339115,'Gionee': 4.349177705,'Google': 4.870146421,
                      'HTC': 4.432936787999999,'Honor': 4.6833263,'Huawei': 4.69015451,'Karbonn': 3.654546775,
                      'LG': 4.322143925,'Lava': 3.9323964145000003,'Lenovo': 4.423648309,'Meizu': 4.533451438,
                      'Micromax': 3.867443962,'Microsoft': 4.195245147,'Motorola': 4.38256042,'Nokia': 4.052654135,
                      'OnePlus': 4.679163866,'Oppo': 4.69701984,'Others': 4.2107191964999995,'Panasonic': 4.282206299,
                      'Realme': 4.668802046,'Samsung': 4.51008998,'Sony': 4.527100531,'Spice': 3.6704561995000002,
                      'Vivo': 4.761831996,'XOLO': 3.947337803,'Xiaomi': 4.630935394,'ZTE': 4.360214212500001}
        Device_Brand = request.form['Device_Brand']
        Device_Brand = brand_dict[Device_Brand]

        test_set = np.array([[screeen_size,FourG,FiveG,rear_camera_mp,front_camera_mp,internal_memory,
                                    ram,battery,weight,days_used,normalized_new_price,Device_Brand,
                                    year2014,year2015,year2016,year2017,year2018,year2019,year2020]])
        test_set = scaler.transform(test_set)
        prediction = model.predict(test_set)
        if new_price<=10000:
            prediction = np.exp(prediction)*7
        elif new_price>10000 and new_price<=16000:
            prediction = np.exp(prediction)*10
        elif new_price>16000 and new_price<=22000:
            prediction = np.exp(prediction)*13
        elif new_price>22000 and new_price<=27000:
            prediction = np.exp(prediction)*16
        elif new_price>27000 and new_price<=49999:
            prediction = np.exp(prediction)*25
        elif new_price>=50000 and new_price<=70000:
            prediction = np.exp(prediction)*30
        elif new_price>70000 and new_price<=1000000:
            prediction = np.exp(prediction)*40

        return render_template('predict.html', prediction_text="Current price of phone is Rs.{}".format(prediction))
 
if __name__ == '__main__': 
    app.run(host='0.0.0.0')


